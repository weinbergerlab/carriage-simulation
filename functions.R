...
prev.lag.func<-function(ds){
  ...
  jags.func<-function(ds){
      ...
      for(k in 2:6){
      prob_2_1[k]<- exp(beta_2_1*time.increment[k]/14) #give probaility for 14 days
      prob_2_3[k]<- exp(beta_2_3*time.increment[k]/14)
      prob_1_2[k]<- exp(beta_1_2*time.increment[k]/14)
      }
    ...
    #exp(beta) gives prob change per 2 weeks
      ...
      posterior_samples<-coda.samples(model_jags, 
                                      variable.names=c('prob_2_1_day', 'prob_2_3_day','prob_1_2_day','beta_2_1','beta_2_3','beta_1_2'),
